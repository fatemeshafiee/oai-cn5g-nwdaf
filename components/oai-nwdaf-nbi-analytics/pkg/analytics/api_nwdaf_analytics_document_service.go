...
type EngineReqData struct {
	...
	// In order to get mobility information for an given imsi
...
func (s *NWDAFAnalyticsDocumentApiService) GetNWDAFAnalytics(ctx context.Context, eventId EventIdAnyOf, anaReq EventReportingRequirement, eventFilter EventFilter, supportedFeatures string, tgtUe TargetUeInformation) (ImplResponse, error) {

	// create AnalyticsData Report
	analyticsData := AnalyticsData{}

	// check event_id not empty
	if eventId == "" {
		return Response(http.StatusBadRequest, ProblemDetails{}), errors.New("missing Event Id param")
	}
	...
	case EVENTIDANYOF_NETWORK_PERFORMANCE:
		...
		if len(nwPerfAnalyticsData) == 0 {
			return Response(http.StatusBadRequest, ProblemDetails{}), errors.New("missing Network Performance Data")
		}
		// set nw_perf field
	...
	case EVENTIDANYOF_UE_COMMUNICATION:
		...
		if len(ueCommsAnalyticsData) == 0 {
			return Response(http.StatusBadRequest, ProblemDetails{}), errors.New("missing UE Communications Data")
		}
		// set ue comms field
	...
	default:
		return Response(http.StatusBadRequest, ProblemDetails{}), errors.New("invalid Event Id param")
	}

	analyticsData.AnaMetaInfo.DataWindow.StartTime = anaReq.StartTs
	analyticsData.AnaMetaInfo.DataWindow.StopTime = anaReq.EndTs

	log.Printf("Returning complete NWDAF Analytics Data")
...
func getNwPerfAnalytics(anaReq EventReportingRequirement, eventFilter EventFilter) ([]NetworkPerfInfo, error) {
	...
	for _, nwPerfType := range eventFilter.NwPerfTypes {
		...
		// check if network performance type is valid
		...
		case NETWORKPERFTYPEANYOF_NUM_OF_UE:
			...
			nwPerfInfo, err = requestNwPerfEngine(anaReq, eventFilter, os.Getenv("ENGINE_URI")+os.Getenv("ENGINE_NUM_OF_UE_ROUTE"))
			if err != nil {
		...
		case NETWORKPERFTYPEANYOF_SESS_SUCC_RATIO:
			...
			nwPerfInfo, err = requestNwPerfEngine(anaReq, eventFilter, os.Getenv("ENGINE_URI")+os.Getenv("ENGINE_SESS_SUCC_RATIO_ROUTE"))
			if err != nil {
				return nwPerfList, err
			}
...
func getUeCommsAnalytics(anaReq EventReportingRequirement, eventFilter EventFilter) ([]UeCommunication, error) {
	...
	ueCommInfo, err = requestUeCommEngine(anaReq, eventFilter, os.Getenv("ENGINE_URI")+os.Getenv("ENGINE_UE_COMMUNICATION_ROUTE"))
	if err != nil {
...
// ------------------------------------------------------------------------------
// getUeCommNotifData - Get list Ue Communication
func getUeMobAnalytics(anaReq EventReportingRequirement, eventFilter EventFilter, tgtUe TargetUeInformation) ([]UeMobility, error) {
	...
	for _, supi := range tgtUe.Supis {
		...
		ueMobInfo, err = requestUeMobEngine(anaReq, eventFilter, supi, os.Getenv("ENGINE_URI")+os.Getenv("ENGINE_UE_MOBILITY_ROUTE"))
		if err != nil {
			return ueMobList, err
		}

		// we need to add supi to the response ?
		// ueMobInfo.Supi = supi
...
// ------------------------------------------------------------------------------
func requestNwPerfEngine(anaReq EventReportingRequirement, eventFilter EventFilter, enginePath string) (NetworkPerfInfo, error) {
	...
	req, err := http.NewRequest(http.MethodGet, enginePath, bytes.NewBuffer(engineReqJsonData))
	if err != nil {
		return NetworkPerfInfo{}, err
	}
...
// ------------------------------------------------------------------------------
func requestUeCommEngine(anaReq EventReportingRequirement, eventFilter EventFilter, enginePath string) (UeCommunication, error) {
...
// ------------------------------------------------------------------------------
func requestUeMobEngine(anaReq EventReportingRequirement, eventFilter EventFilter, supi string, enginePath string) (UeMobility, error) {
	...
	for _, userLocation := range ueMobResp.Loc {
		// Create a LocationInfo struct for each UserLocation
		...
		// Append the LocationInfo to the LocInfos slice in UeMobility
