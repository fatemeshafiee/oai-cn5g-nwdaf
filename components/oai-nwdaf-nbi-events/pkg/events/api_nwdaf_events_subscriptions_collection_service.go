...
func (s *NWDAFEventsSubscriptionsCollectionApiService) CreateNWDAFEventsSubscription(ctx context.Context, nnwdafEventsSubscription NnwdafEventsSubscription, urlPath string) (ImplResponse, error) {

	// TODO
	// run tests about feasibility of creating subscription according to TS29520 rules and update subscription info

	// -----------------------------------------------
	// create subscription and go routine
	// -----------------------------------------------

	// create new subscription id
	subscriptionId := uuid.New().String()

	// create bool channel to control go routine to stop
	...
	for _, eventSub := range nnwdafEventsSubscription.EventSubscriptions {
		...
		go handleSubscriptionEvent(ctx, eventSub, nnwdafEventsSubscription.NotificationURI, subscriptionId, subscriptionCh)

	}

	// -----------------------------------------------
	// prepare http reponse
	// -----------------------------------------------

	// add location header in http response
	respHeaders := make(map[string][]string)
	respHeaders["Location"] = []string{urlPath + "/" + subscriptionId}

	// fill http response body
	// TODO
	// implement FailEventReports when events aren't accepted, and event Request to get reports of events.
...
// ------------------------------------------------------------------------------
// send only accepted events to handle subscription after feasibility check (in nnwdafEventsSubscription)
func handleSubscriptionEvent(ctx context.Context, eventSub EventSubscription, notificationURI string, subscriptionId string, subscriptionCh <-chan string) {

	log.Print("Handling subscription to ", eventSub.Event, " with subscription id ", subscriptionId)
...
loop:
	...
	log.Print("subscription to ", eventSub.Event, " with subscription id ", subscriptionId, " is closed.")
}

// ------------------------------------------------------------------------------
// fillEventNotification - return event notification information
func fillEventNotification(ctx context.Context, eventSub EventSubscription) (EventNotification, error) {
...
func getNwPerfNotifData(eventSub EventSubscription) ([]NetworkPerfInfo, error) {
	...
	for _, nwPerfReq := range eventSub.NwPerfRequs {
		...
		case NETWORKPERFTYPE_NUM_OF_UE:
			...
			nwPerfInfo, err = requestNwPerfEngine(eventSub, os.Getenv("ENGINE_URI")+os.Getenv("ENGINE_NUM_OF_UE_ROUTE"))
			if err != nil {
		...
		case NETWORKPERFTYPE_SESS_SUCC_RATIO:
			...
			nwPerfInfo, err = requestNwPerfEngine(eventSub, os.Getenv("ENGINE_URI")+os.Getenv("ENGINE_SESS_SUCC_RATIO_ROUTE"))
...
func getUeCommNotifData(eventSub EventSubscription) ([]UeCommunication, error) {
	...
	ueCommInfo, err = requestUeCommEngine(eventSub, os.Getenv("ENGINE_URI")+os.Getenv("ENGINE_UE_COMMUNICATION_ROUTE"))
	if err != nil {
...
func getUeMobNotifData(eventSub EventSubscription) ([]UeMobility, error) {
	...
	for _, supi := range eventSub.TgtUe.Supis {
		...
		ueMobility, err = requestUeMobEngine(eventSub, supi, os.Getenv("ENGINE_URI")+os.Getenv("ENGINE_UE_MOBILITY_ROUTE"))
		if err != nil {
...
func getAbnormalBehaviourNotifData(eventSub EventSubscription) ([]AbnormalBehaviour, error) {
	...
	for _, excepReq := range eventSub.ExcepRequs {
		...
		case EXCEPTIONID_UNEXPECTED_LARGE_RATE_FLOW:
			...
			AbnorBehavrsInfo, err = requestAbnorBehavrsEngine(eventSub, excepReq, os.Getenv("ENGINE_ADS_URI")+os.Getenv("ENGINE_UNEXPECTED_LARGE_RATE_FLOW_ROUTE"))
			if err != nil {
...
// ------------------------------------------------------------------------------
func sendNotification(ctx context.Context, eventNotif EventNotification, notificationURI string) error {
...
// ------------------------------------------------------------------------------
func requestNwPerfEngine(eventSub EventSubscription, enginePath string) (NetworkPerfInfo, error) {
...
// ------------------------------------------------------------------------------
func requestUeCommEngine(eventSub EventSubscription, enginePath string) (UeCommunication, error) {
...
// ------------------------------------------------------------------------------
func requestUeMobEngine(eventSub EventSubscription, supi string, enginePath string) (UeMobility, error) {
...
// ------------------------------------------------------------------------------
func requestAbnorBehavrsEngine(eventSub EventSubscription, excepReq Exception, enginePath string) (AbnormalBehaviour, error) {
	...
	log.Println("unexpected_large_rate_flow probability is: ", float64(abnorBehavrsResp.Ratio)/float64(100))
